...
void externalise_spans(parse_tree *tree) {
  span *const spans = malloc(sizeof(span) * tree->data.node_amt);
  const node_ind_t node_amt = tree->data.node_amt;
  parse_node *const nodes = tree->data.nodes;
