...
// TODO test
void *memmem(const void *restrict haystack, size_t haystack_len,
             const void *restrict needle, size_t needle_len) {
  const char *hs = (const char *)haystack;
  const char *nd = (const char *)needle;
  if (haystack == NULL || haystack == NULL || needle_len > haystack_len ||
